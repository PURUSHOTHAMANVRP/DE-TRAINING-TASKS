...
def read_sheet_metrics_as_dict(excel_path, sheet_name):
    ...
    df = df[df["metric"].notna() & (df["metric"] != "") & (df["metric"].str.lower() != "nan")]
...
def find_month_column(ws, month_header):
    target = month_header.strip().lower()
...
def main():
    ...
    sheet_to_month = {
        next(s for s in input_sheets if s.strip().startswith("MI - Apr")): "Apr-25",
        "May": "May-25",
        "June": "Jun-25",
    }
    ...
    print(f"\n Updated successfully: {OUTPUT_FILE}")
